<view>
    ...
    <img data-src="{{image.0.img}}" alt="header"
        class="img-fluid w-{{imgwidth}} float-{{imgpos}} mb-{{imgmrg}} {{_var.mrg}}" wb-if="'{{image.0.img}}' > ''" />

    <h2 wb-if="'{{header}}' > ''">{{header}}</h2>
    {{text}}

    <wb-jq wb="$dom->find('h2')->addClass('post__subtitle')" />
</view>

<edit header="Блок с контентом">
    <div>
    ...
    <wb-multilang wb-lang="{{_sett.locales}}" name="lang">
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-4">
                <wb-module wb="module=filepicker&amp;mode=single&amp;width=150&amp;height=100" name="image"></wb-module>
            </div>
        </div>
    </wb-multilang>
    <wb-lang>
